...
def train_model(model, criterion, optimizer, scheduler, num_epochs, use_gpu):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['trainImages', 'testImages']:
            if phase == 'trainImage':
                scheduler.step()
...
if __name__ == '__main__':
    ...
    image_datasets = {x: customData(img_path='/export/home/dyh/workspace/circle_k/for_douyuhao/data',
                                    txt_path=(x + '.txt'),
                                    data_transforms=data_transforms,
                                    dataset=x) for x in ['trainImages', 'testImages']}

    # wrap your data and label into Tensor
    dataloders = {x: torch.utils.data.DataLoader(image_datasets[x],
                                                 batch_size=batch_size,
                                                 shuffle=True) for x in ['trainImages', 'testImages']}

    dataset_sizes = {x: len(image_datasets[x]) for x in ['trainImages', 'testImages']}
