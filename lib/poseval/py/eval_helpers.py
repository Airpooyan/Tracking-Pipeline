...
import json
import glob
from poseval.py.convert import convert_videos
...
def cleanupData(gtFramesAll,prFramesAll):
  ...
  imgidxs = []
  #print(gtFramesAll.keys())
  #print(prFramesAll.keys())
...
def load_data_dir(argv):
  ...
  for i in range(len(filenames)):
    ...
    with open(predFilename) as data_file:
        data = json.load(data_file)
#-----------------------------------------------------------------------    
    def keep_thresh(data, thresh =0.2):
        ...
        for annotation in annolist:
            ...
            if len(annorect) == 0:
                ...
                for anno in annorect:
                    points = anno['annopoints'][0]['point']
...
#-----------------------------------------------------------------------
    ...
    pr = data["annolist"]
    if (len(pr) != len(gt)):
        #print('# prediction frames %d <> # GT frames %d for %s' % (len(pr),len(gt),predFilename))
    ...
    for imgidx in range(len(pr)):
        for ridxPr in range(len(pr[imgidx]["annorect"])):
            if ("track_id" in pr[imgidx]["annorect"][ridxPr].keys()):
                # adjust track_ids to make them unique across all sequences
                if isinstance(pr[imgidx]["annorect"][ridxPr]["track_id"],int):
                    pr[imgidx]["annorect"][ridxPr]["track_id"]=[pr[imgidx]["annorect"][ridxPr]["track_id"]]
