...
from track.alexnet_cpm import SiameseAlexNet
...
class SiamFCTracker:
	def __init__(self, gpu_id=0, model_path='/export/home/zby/SiamFC/models/output/siamfc_1.pth'):
		self.gpu_id = gpu_id
		print('----------------------------------------')
	...
	def update_data_dict(self, frame, bbox, id):
		...
		exemplar_pose_img = cv2.resize(exemplar_img_bgr, (368,654), interpolation=cv2.INTER_LINEAR)
		exemplar_pose_img = self.pose_transforms(exemplar_pose_img)[None,:,:,:]
		
		exemplar_img = self.transforms(exemplar_img)[None,:,:,:]   # add new axis
		#print(exemplar_img.shape, exemplar_pose_img.shape)
	...
	def track_id(self, frame, id):
		...
		pose_pyramid = [cv2.resize(x[:,:,::-1], (368,654), interpolation=cv2.INTER_LINEAR) for x in pyramid]
